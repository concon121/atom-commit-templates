...
module.exports = {
  ...
  activate: function () {
    const self = this;
    self.setup();
    atom.commands.add('atom-workspace', 'atom-jira-commits:Set Commit Template', self.setCommitTemplate);
  },
  ...
  setCommitTemplate: function() {
    var dialog = new InputDialog({
      callback: (text) => {
        ...
        fs.writeFile(path.join(ps.env.HOMEPATH, '.gitcommit'), text, (err) => {
          if (err) {
            console.log(err)
          }
        })
